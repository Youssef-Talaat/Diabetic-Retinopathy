...
def getNumOfImagesPerClass():
    ...
    images_num_per_class_original[0] = min(target_images_per_class, ws_original[0].max_row)
    images_num_per_class_sampling[0] = target_images_per_class - images_num_per_class_original[0]

    local_target_images_per_class = target_images_per_class / 4

    for i in range(1, classes_num, 1):
        images_num_per_class_original[i] = min(local_target_images_per_class, ws_original[i].max_row)
        images_num_per_class_sampling[i] = local_target_images_per_class - images_num_per_class_original[i]
...
def writeToMainArrays(distribution_type):
    ...
    if distribution_type == "block":
        ...
        for j in range(train_size_per_class_original[0]):
            train_images[train_counter] = classes_train_images_original[0][j]
            train_labels[train_counter] = 0
            train_counter += 2
        for j in range(train_size_per_class_sampling[0]):
            train_images[train_counter] = classes_train_images_sampling[0][j]
            train_labels[train_counter] = 0
            train_counter += 2
        for j in range(test_size_per_class_original[0]):
            test_images[test_counter] = classes_test_images_original[0][j]
            test_labels[test_counter] = 0
            test_counter += 2
        for j in range(test_size_per_class_sampling[0]):
            test_images[test_counter] = classes_test_images_sampling[0][j]
            test_labels[test_counter] = 0
            test_counter += 2
        ...
        for i in range(1, classes_num, 1):
            ...
            for j in range(train_size_per_class_original[i]):
                if class_index == 5:
                    class_index = 1
                train_images[train_counter] = classes_train_images_original[class_index][j]
                train_labels[train_counter] = 1
                train_counter += 4
                class_index += 1
            ...
            for j in range(train_size_per_class_sampling[i]):
                if class_index == 5:
                    class_index = 1
                train_images[train_counter] = classes_train_images_sampling[class_index][j]
                train_labels[train_counter] = 1
                train_counter += 4
                class_index += 1
            ...
            for j in range(test_size_per_class_original[i]):
                if class_index == 5:
                    class_index = 1
                test_images[test_counter] = classes_test_images_original[class_index][j]
                test_labels[test_counter] = 1
                test_counter += 4
                class_index += 1
            ...
            for j in range(test_size_per_class_sampling[i]):
                if class_index == 5:
                    class_index = 1
                test_images[test_counter] = classes_test_images_sampling[class_index][j]
                test_labels[test_counter] = 1
                test_counter += 4
                class_index += 1
...
def createModel():
    ...
    x = Dropout(0.5)(x)
    headModel = Dense(classification_classes, activation='sigmoid', kernel_regularizer='l2')(x)
...
def evaluateModel(_model):
    ...
    model.save('YesNo_2Classes_fullModel.h5')
    model.save_weights('YesNo_2Classes_weights.h5')
...
print("Original Images Number = 35480")
