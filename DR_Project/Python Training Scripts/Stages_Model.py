...
train_images = np.zeros((np.amin(class_train_size)*4*4, image_dimension, image_dimension, 3), dtype='uint8')
train_labels = np.zeros((np.amin(class_train_size)*4*4, 1), dtype='uint8')

test_images = np.zeros((np.amin(class_test_size)*4*4, image_dimension, image_dimension, 3), dtype='uint8')
test_labels = np.zeros((np.amin(class_test_size)*4*4, 1), dtype='uint8')
...
index = 0
for i in range(5324):
    train_images[index] = c1_train_all[i]
    train_labels[index] = 0
    index += 1
    train_images[index] = c2_train_all[i]
    train_labels[index] = 1
    index += 1
    train_images[index] = c3_train_all[i]
    train_labels[index] = 2
    index += 1
    train_images[index] = c4_train_all[i]
    train_labels[index] = 3
    index += 1

index = 0
for i in range(2280):
    test_images[index] = c1_test_all[i]
    test_labels[index] = 0
    index += 1
    test_images[index] = c2_test_all[i]
    test_labels[index] = 1
    index += 1
    test_images[index] = c3_test_all[i]
    test_labels[index] = 2
    index += 1
    test_images[index] = c4_test_all[i]
    test_labels[index] = 3
    index += 1
...
print(c1_train_all.size + c1_test_all.size +
      c2_train_all.size + c2_test_all.size +
      c3_train_all.size + c3_test_all.size +
      c4_train_all.size + c4_test_all.size)
...
x = Dropout(0.5)(x)
headModel = Dense(4, activation='softmax')(x)
