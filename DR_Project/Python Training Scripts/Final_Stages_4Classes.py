...
def readImagesToArrays():
    ...
    temp0 = np.zeros(((images_num_per_class_original[1] + images_num_per_class_sampling[1]), image_dimension, image_dimension, 3), dtype='uint8')
    temp1 = np.zeros(((images_num_per_class_original[2] + images_num_per_class_sampling[2]), image_dimension, image_dimension, 3), dtype='uint8')
    temp2 = np.zeros(((images_num_per_class_original[3] + images_num_per_class_sampling[3]), image_dimension, image_dimension, 3), dtype='uint8')
    temp3 = np.zeros(((images_num_per_class_original[4] + images_num_per_class_sampling[4]), image_dimension, image_dimension, 3), dtype='uint8')
    images = np.array([temp0, temp1, temp2, temp3])

    for i in range(classification_classes):
        counter = 0
        print("1st Image Name Original", ws_original[i + 1].cell(1, 1).value)
        print("1st Image Name Sampling", ws_sampling[i + 1].cell(1, 1).value)
        for j in range(images_num_per_class_original[i + 1]):
            images[i][counter] = cv2.imread(original_images_path + str(ws_original[i + 1].cell(j+1, 1).value))
            counter += 1
        for j in range(images_num_per_class_sampling[i + 1]):
            images[i][counter] = cv2.imread(sampling_images_path + str(ws_sampling[i + 1].cell(j+1, 1).value))
            counter += 1
...
def splitData():
    print("splitData()")
    for c in range(classification_classes):
        train_size_per_class_original[c] = int(images_num_per_class_original[c + 1] * train_percentage + 0.5)
        test_size_per_class_original[c] = images_num_per_class_original[c + 1] - train_size_per_class_original[c]
        print(str(train_size_per_class_original[c]) + " , " + str(test_size_per_class_original[c]))

        train_size_per_class_sampling[c] = int(images_num_per_class_sampling[c + 1] * train_percentage + 0.5)
        test_size_per_class_sampling[c] = images_num_per_class_sampling[c + 1] - train_size_per_class_sampling[c]
    ...
    classes_test_images_local_original = np.array([classes_images[0][train_size_per_class_original[0]:images_num_per_class_original[1]],
                                                   classes_images[1][train_size_per_class_original[1]:images_num_per_class_original[2]],
                                                   classes_images[2][train_size_per_class_original[2]:images_num_per_class_original[3]],
                                                   classes_images[3][train_size_per_class_original[3]:images_num_per_class_original[4]]])

    classes_train_images_local_sampling = np.array([classes_images[0][images_num_per_class_original[1]:(images_num_per_class_original[1] + train_size_per_class_sampling[0])],
                                                    classes_images[1][images_num_per_class_original[2]:(images_num_per_class_original[2] + train_size_per_class_sampling[1])],
                                                    classes_images[2][images_num_per_class_original[3]:(images_num_per_class_original[3] + train_size_per_class_sampling[2])],
                                                    classes_images[3][images_num_per_class_original[4]:(images_num_per_class_original[4] + train_size_per_class_sampling[3])]])
    classes_test_images_local_sampling = np.array([classes_images[0][(images_num_per_class_original[1] + train_size_per_class_sampling[0]):],
                                                   classes_images[1][(images_num_per_class_original[2] + train_size_per_class_sampling[1]):],
                                                   classes_images[2][(images_num_per_class_original[3] + train_size_per_class_sampling[2]):],
                                                   classes_images[3][(images_num_per_class_original[4] + train_size_per_class_sampling[3]):]])
...
def writeToMainArrays(distribution_type):
    ...
    if distribution_type == "block":
        ...
        for i in range(classification_classes):
            print("i = ", i)
            print("train_size_per_class_original = ", train_size_per_class_original[i])
            print("train_size_per_class_sampling = ", train_size_per_class_sampling[i])
            print("test_size_per_class_original = ", test_size_per_class_original[i])
            print("test_size_per_class_sampling = ", test_size_per_class_sampling[i])
            train_counter = i
            test_counter = i
            for j in range(train_size_per_class_original[i]):
                ...
                train_labels[train_counter] = i
                train_counter += classification_classes
            ...
            for j in range(test_size_per_class_original[i]):
                ...
                test_labels[test_counter] = i
                test_counter += classification_classes
...
def createModel():
    ...
    x = Dropout(0.5)(x)
    headModel = Dense(classification_classes, activation='softmax', kernel_regularizer='l2')(x)
...
def evaluateModel(_model):
    ...
    model.save('Stages_4Classes_fullModel.h5')
    model.save_weights('Stages_4Classes_weights.h5')
...
print("Classes =", classification_classes)
...
train_size_per_class_original = np.zeros(classification_classes, int)
test_size_per_class_original = np.zeros(classification_classes, int)

train_size_per_class_sampling = np.zeros(classification_classes, int)
test_size_per_class_sampling = np.zeros(classification_classes, int)
